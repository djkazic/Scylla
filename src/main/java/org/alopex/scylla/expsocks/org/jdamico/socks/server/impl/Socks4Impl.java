...
public class Socks4Impl implements SocksCommonInterface {
	...
	public ProxySocketHandler m_Parent	= null;
	...
	public	Socks4Impl( ProxySocketHandler Parent )	{
	...
	//	Connect to the Remote Host
		try	{
			...
			DebugLog.getInstance().getSocketInfo( m_Parent.serverSocket) );
		}
		
		DebugLog.getInstance().println( "Connected to "+DebugLog.getInstance().getSocketInfo( m_Parent.serverSocket) );
	...
	public void	bind()	throws IOException
	{
		...
		m_Parent.serverSocket = socket;
		m_Parent.prepareServer();
		
		DebugLog.getInstance().println( "BIND Connection from "+DebugLog.getInstance().getSocketInfo( m_Parent.serverSocket) );
		ssock.close();
