...
import org.alopex.scylla.expsocks.org.jdamico.socks.server.commons.Utils;
...
//	public			byte[]	DST_Port;		// Destination Port
	...
	public	Socks5Impl( ProxySocketHandler Parent )	{
	...
	public	boolean	calculateAddress()	{
		...
		m_ClientIP		= m_Parent.clientSocket.getInetAddress();
		m_nClientPort	= m_Parent.clientSocket.getPort();
	...
	public	void	replyCommand( byte replyCode )	{
		...
		if( m_Parent.serverSocket != null )	{
			//IA = m_Parent.serverSocket.getInetAddress();
			//DN = IA.toString();
			pt = m_Parent.serverSocket.getLocalPort();
		}
		else	{
			IP[0]=0;
			IP[1]=0;
	...
	public	void	udp() throws IOException {
		...
		InetAddress	MyIP   = m_Parent.clientSocket.getLocalAddress();
	...
	private	void	initUdpInOut()	throws IOException {
		...
		m_Parent.buffer = new byte[ Constants.DEFAULT_BUF_SIZE ];
		
		DGPack = new DatagramPacket( m_Parent.buffer, Constants.DEFAULT_BUF_SIZE );
	}
	/////////////////////////////////////////////////////////////
	...
	*/
	...
	public	void	processUdpRemote()	{
		...
		Buf = addDgpHead( m_Parent.buffer);
		if( Buf == null )	return;
